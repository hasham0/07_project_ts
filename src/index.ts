...
let playersAttckEffect: number = 30;
let playerPower: number = 20;

//enimes
const enimes: string[] = ["Thanos", "loki", "kang", "darkside"];
let enimesPower: number = 16;
let enimesheath: number = 50;
let enimesAttckEffect: number = 25;
...
async function gameFunc() {
  ...
  while (run) {
    ...
    if (phases["level"] === "fight") {
      // selecting enemy
      const check: number = Math.floor(Math.random() * (enimes.length - 0) + 0);
